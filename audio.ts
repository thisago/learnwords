...
const downloadFile =
  (outputPath: string) => (url: string) => (name: string) => {
    try {
      return fetch(url, {
        headers: {
...
export const cookie = `__gads=ID=562a6c0aa0c09c2f:T=1593115520:S=ALNI_MbDjUFkW4gq5k41KjroOPnqoj_sWA; _ga=GA1.2.496721152.1593115521; _gid=GA1.2.1365538900.1593115521; _fbp=fb.1.1593115521224.1750570304; __stripe_mid=26abf6e7-ef1f-41b8-97cf-b27996194d74; _gat_gtag_UA_28351091_23=1`
...
export const newAudio = (cookie: string) => (lang: string) => (value: string) => {
  const getLang = (lang: string): string => {
    ...
    return langs[lang2[0]][
      Math.floor(Math.random() * (langs[lang2[0]].length - 1))
    ]
  }
  const language = getLang(lang)

  const body = `msg=${encodeURIComponent(value)}&lang=${encodeURIComponent(
    language
  )}&source=ttsmp3`

  const filename = `${value.replace(/ /g, `_`)}-${language}-${randomString(
    10
  )}.mp3`
  ...
  try {
    fetch(`https://ttsmp3.com/makemp3_new.php`, {
      ...
      .then((res) => res.json())
      .then((res) => downloadMp3(res.URL)(filename))
...
const addLineToWordlist = (file: string) => (newLine: Word) => {
  var oldFileValue: Word[] = JSON.parse(
